...
def augment(staging_collection, categories_collection, parties_collection, politicians_collection):
    ...
    # Wherever lead is empty, fill it up with first two sentences from the body content
    ...
    # If there is any political reference in the article meaning, any party name or political person is mentioned in the 
    # article then the category of that article would be changed to Panorama only if the cateogory of that article is not listed
    # categories collection
    update_primay_category(staging_collection, categories_collection)

    # # take the image url from teh categories collection and update the url in staging against the article based on its category
    update_image_urls(staging_collection, categories_collection)
    

    # # Validate the article based on certain condition
    # # if it passes through all the conditions, only then it will be pushed to Articles collection 
...
def fill_lead_from_article_body(staging_collection):
    ...
    # Only pull the articles where lead is empty and the body length is  more than and equal to 150     
...
def findPoliticalReferences(staging_collection, parties_collection, politicians_collection):
    ...
    # in a specific order we are maintainig the list manually
        ...
        for party_category in party_categories:
            ...
            for party in parties_cursor:                
                ...
                if (_party_names_found != ''):
                    ...
                    # If party is mentioned, only then its politicians will be checked in the articles. Otherwise No.
...
def search_party_in_article(article, party):
    ...
    # party name is not part of Aliases, so add it to the aliases list so that can be matched as well
...
def search_politician_in_article(article, politician):
    ...
    #if full name is equal to (first + last) name then apply the regex_lastname_with_firstname
...
def update_primay_category(staging_collection, categories_collection):
    ...
    for staging_article in staging_cursor:
        ...
        if staging_article['primary_category'] == "":
            primary_category = 0 # means this does not exist in categories collection
        else:
...
def update_image_urls(staging_collection, categories_collection):
    ...
    for staging_article in staging_cursor:
        ...
        # only update the image url for the articles whose categories are present in "categories" collection. 
        if category is not None:        
            updated_values = { "$set": { "image_url": category['image_url'] } }
            update_query = { "_id": staging_article['_id'] }
            staging_collection.update_one(update_query, updated_values)

    # fill the image_url with empty string for all the articles whose category is not listed in the categories collection.
